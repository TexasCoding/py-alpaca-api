...
def account_class_from_dict(data_dict):
    return AccountClass(
        ...
        admin_configurations=(
            object(data_dict["admin_configurations"])
            if data_dict["admin_configurations"]
            else {}
        ),
        user_configurations=(
            object(data_dict["user_configurations"])
            if data_dict["user_configurations"]
            else {}
        ),
        account_number=str(data_dict["account_number"]),
        status=str(data_dict["status"]) if data_dict["status"] else "",
        crypto_status=(
            str(data_dict["crypto_status"])
            if data_dict["crypto_status"]
            else ""
        ),
        options_approved_level=(
            int(data_dict["options_approved_level"])
            if data_dict["options_approved_level"]
            else None
        ),
        options_trading_level=(
            int(data_dict["options_trading_level"])
            if data_dict["options_trading_level"]
            else None
        ),
        currency=str(data_dict["currency"]) if data_dict["currency"] else "",
        buying_power=(
            float(data_dict["buying_power"])
            if data_dict["buying_power"]
            else None
        ),
        regt_buying_power=(
            float(data_dict["regt_buying_power"])
            if data_dict["regt_buying_power"]
            else None
        ),
        daytrading_buying_power=(
            float(data_dict["daytrading_buying_power"])
            if data_dict["daytrading_buying_power"]
            else None
        ),
        effective_buying_power=(
            float(data_dict["effective_buying_power"])
            if data_dict["effective_buying_power"]
            else None
        ),
        non_marginable_buying_power=(
            float(data_dict["non_marginable_buying_power"])
            if data_dict["non_marginable_buying_power"]
            else None
        ),
        options_buying_power=(
            float(data_dict["options_buying_power"])
            if data_dict["options_buying_power"]
            else None
        ),
        bod_dtbp=(
            float(data_dict["bod_dtbp"]) if data_dict["bod_dtbp"] else None
        ),
        cash=float(data_dict["cash"]) if data_dict["cash"] else None,
        accrued_fees=(
            float(data_dict["accrued_fees"])
            if data_dict["accrued_fees"]
            else None
        ),
        pending_transfer_in=(
            float(data_dict["pending_transfer_in"])
            if data_dict["pending_transfer_in"]
            else None
        ),
        portfolio_value=(
            float(data_dict["portfolio_value"])
            if data_dict["portfolio_value"]
            else None
        ),
        ...
        multiplier=(
            int(data_dict["multiplier"]) if data_dict["multiplier"] else None
        ),
        shorting_enabled=bool(data_dict["shorting_enabled"]),
        equity=float(data_dict["equity"]) if data_dict["equity"] else None,
        last_equity=(
            float(data_dict["last_equity"])
            if data_dict["last_equity"]
            else None
        ),
        long_market_value=(
            float(data_dict["long_market_value"])
            if data_dict["long_market_value"]
            else None
        ),
        short_market_value=(
            float(data_dict["short_market_value"])
            if data_dict["short_market_value"]
            else None
        ),
        position_market_value=(
            float(data_dict["position_market_value"])
            if data_dict["position_market_value"]
            else None
        ),
        initial_margin=(
            float(data_dict["initial_margin"])
            if data_dict["initial_margin"]
            else None
        ),
        maintenance_margin=(
            float(data_dict["maintenance_margin"])
            if data_dict["maintenance_margin"]
            else None
        ),
        last_maintenance_margin=(
            float(data_dict["last_maintenance_margin"])
            if data_dict["last_maintenance_margin"]
            else None
        ),
        sma=float(data_dict["sma"]) if data_dict["sma"] else None,
        daytrade_count=(
            int(data_dict["daytrade_count"])
            if data_dict["daytrade_count"]
            else None
        ),
        balance_asof=(
            str(data_dict["balance_asof"]) if data_dict["balance_asof"] else ""
        ),
        crypto_tier=(
            int(data_dict["crypto_tier"]) if data_dict["crypto_tier"] else None
        ),
        intraday_adjustments=(
            int(data_dict["intraday_adjustments"])
            if data_dict["intraday_adjustments"]
            else None
        ),
        pending_reg_taf_fees=(
            float(data_dict["pending_reg_taf_fees"])
...
def order_class_from_dict(data_dict):
    return OrderClass(
        ...
        replaced_by=(
            data_dict["replaced_by"].split(".")[0].replace("T", " ")
            if data_dict["replaced_by"]
            else ""
        ),
        ...
        symbol=str(data_dict["symbol"]) if data_dict["symbol"] else "",
        asset_class=(
            str(data_dict["asset_class"]) if data_dict["asset_class"] else ""
        ),
        notional=float(data_dict["notional"]) if data_dict["notional"] else 0,
        qty=float(data_dict["qty"]) if data_dict["qty"] else 0,
        filled_qty=(
            float(data_dict["filled_qty"]) if data_dict["filled_qty"] else 0
        ),
        filled_avg_price=(
            float(data_dict["filled_avg_price"])
            if data_dict["filled_avg_price"]
            else 0
        ),
        order_class=(
            str(data_dict["order_class"]) if data_dict["order_class"] else ""
        ),
        order_type=(
            str(data_dict["order_type"]) if data_dict["order_type"] else ""
        ),
        type=str(data_dict["type"]) if data_dict["type"] else "",
        side=str(data_dict["side"]) if data_dict["side"] else "",
        time_in_force=(
            str(data_dict["time_in_force"])
            if data_dict["time_in_force"]
            else ""
        ),
        limit_price=(
            float(data_dict["limit_price"]) if data_dict["limit_price"] else 0
        ),
        stop_price=(
            float(data_dict["stop_price"]) if data_dict["stop_price"] else 0
        ),
        status=str(data_dict["status"]) if data_dict["status"] else "",
        extended_hours=bool(data_dict["extended_hours"]),
        legs=object(data_dict["legs"]) if data_dict["legs"] else {},
        trail_percent=(
            float(data_dict["trail_percent"])
            if data_dict["trail_percent"]
            else 0
        ),
        trail_price=(
            float(data_dict["trail_price"]) if data_dict["trail_price"] else 0
        ),
